...
// jsdom: doc state only; no layout. testing-library/preact runs effects.
...
describe('useCodeMirror', () => {
  ...
  it('language plain omits JSON parse/highlight extensions but still edits', () => {
    function PlainEditor({ value, onChange }) {
      const ref = useCodeMirror(value, onChange, { language: 'plain' });
      return <div ref={ref} data-testid="cm-root" style={{ height: '150px' }} />;
    }
    const onChange = vi.fn();
    const { getByTestId } = render(<PlainEditor value="q=*:*&rows=10" onChange={onChange} />);
    const view = getByTestId('cm-root').__cmView;
    expect(view.state.doc.toString()).toBe('q=*:*&rows=10');
    view.dispatch({ changes: { from: 0, to: 0, insert: 'x' } });
    expect(onChange).toHaveBeenCalledWith('xq=*:*&rows=10');
  });
